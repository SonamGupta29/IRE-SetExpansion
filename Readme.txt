...
Once the program is run it first creates a primary index file for the Google dataset. This primary index file contains the word along with its vector. There are around 3 million words for which the vectors are stored. As this file is very large so in order to enable fast access in this file we create a secondary index file containing the word along with its offset in the primary index file. 
...
Once the cosine distance for all words have been found we choose the top 'n' words which are closest to the input words and they are displayed.
