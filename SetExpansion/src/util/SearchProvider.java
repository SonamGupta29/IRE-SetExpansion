...
public class SearchProvider {
	...
	private static final String APP_ID = "rLSasvRW9cvlU5fG9hoSGjJG2M1eIjR+Ld27nFC9Pj8="; // mrugani
	static {
	...
	public static ArrayList<WebPage> bingSearch(ArrayList<String> seedList,
		...
		if (results == null) {
			...
			java.util.List<api.BingSearchAPI.results> results1 = bingSearchAPI.getTopURLs(constructQuery(seedList, concept), 10);
			int i=0;
			for(api.BingSearchAPI.results r: results1){
				...
				WebPage page = new WebPage(r.Title, r.Url,
						r.Description);
				ArrayList<String> webList = new ArrayList<String>();
				while ((webList = myfinder.getNextList()) != null
						&& webList.size() > 0) {
					if (ListUtil.getOverLap(webList, seedList) >= (seedList
							.size() * overlapTolerance)) {
						page.addList(new WebList(webList, myfinder.getHeader(),
								myfinder.getDescription()));
					}
				}
				listPages.add(page);
			}
			...
			for (AzureSearchWebResult r : results) {
				...
				WebPage page = new WebPage(r.getTitle(), r.getUrl(),
						r.getDescription());
				ArrayList<String> webList = new ArrayList<String>();
				while ((webList = myfinder.getNextList()) != null
						&& webList.size() > 0) {
