...
public class Extractor {
	...
	public static String getConcept(ArrayList<String> seedList, String concept,
		...
		System.out.println("no of urls found for concept: " + listpages.size());

		for (WebPage wp : listpages) {
			// titles
			// Set<String> uniq = new HashSet<String>(Arrays.asList(arr));
			ArrayList<String> uniq = new ArrayList<String>(Arrays.asList(wp
					.getTitle().split(IRUtil.Token)));
			// weight--;
			update(uniq, seedList, weight, wp.getUrl());

			// snippest
			// Set<String> uniq = new HashSet<String>(Arrays.asList(arr));
			if (wp.getSnippest() != null) {
				uniq = new ArrayList<String>(Arrays.asList(wp.getSnippest()
						.split(IRUtil.Token)));
				update(uniq, seedList, weight, wp.getUrl());
			}
			// urls
			...
			for (WebList wl : wp.getAllList()) {
				uniq = new ArrayList<String>(Arrays.asList(wl.getHeading()
						.split(IRUtil.Token)));
				update(uniq, seedList, weight, wp.getUrl());

				uniq = new ArrayList<String>(Arrays.asList(wl.getDescription()
						.split(IRUtil.Token)));
				update(uniq, seedList, weight, wp.getUrl());
			}
		}
		...
		while (count++ < 3) {
			...
			try{
				Integer.parseInt(candidateConcept);
			...
			LogUtil.log.info("Trying out concept " + candidateConcept
					+ " : got score :" + s);
	...
	private static double getConceptScore(String candidateConcept,
		...
		ArrayList<String> concept = SearchProvider.getURLs(mockSeeds, null,
				noOfResults, 1);
		ArrayList<String> seeds = SearchProvider.bingUrls(seedList, null,
				noOfResults, 1);
		...
		LogUtil.log.fine(seedList + " : " + candidateConcept + " scored = "
				+ score);
		return score;
	}

	public static void update(ArrayList<String> uniq,
			ArrayList<String> seedList, int weight, String url) {
	...
	public static int checkValidTokens(String word, ArrayList<String> seedList){
		...
		for (String seed : seedList) {
			if (seed.contains(smallWord) || smallWord.contains(seed)) {
				LogUtil.log.finer("Word " + smallWord
						+ " is already part of seed " + seed);
				return 1;
			}
		}
