...
public class Word2VecModel {
	...
	public Word2VecModelThrift toThrift() {
		return new Word2VecModelThrift().setVocab(vocab)
				.setLayerSize(layerSize).setVectors(Doubles.asList(vectors));
	}
	...
	public static Word2VecModel fromThrift(Word2VecModelThrift thrift) {
		return new Word2VecModel(thrift.getVocab(), thrift.getLayerSize(),
				Doubles.toArray(thrift.getVectors()));
	}

	/**
	 * @return {@link Word2VecModel} read from a file in the text output format
	 *         of the Word2Vec C open source project.
	 */
	...
	/**
	 * @return {@link Word2VecModel} created from the binary representation
	 *         output by the open source C version of word2vec using the given
	 *         byte order.
	 */
	public static Word2VecModel fromBinFile(File file, ByteOrder byteOrder)
		...
		try (FileInputStream fis = new FileInputStream(file);) {
			DataInput in = (byteOrder == ByteOrder.BIG_ENDIAN) ? new DataInputStream(
					fis) : new SwappedDataInputStream(fis);
			...
			for (int lineno = 0; lineno < vocabSize; lineno++) {
				...
				double[] m = Doubles.toArray(vectors);
				normalize(m);
				DatabaseConnection.insertWordVectors(word, m);
				vectors.clear();

			}

			return fromThrift(new Word2VecModelThrift().setLayerSize(layerSize)
					.setVocab(vocabs).setVectors(vectors));
		}
	}
	...
	/**
	 * @return {@link Word2VecModel} from the lines of the file in the text
	 *         output format of the Word2Vec C open source project.
	 */
	@VisibleForTesting
	static Word2VecModel fromTextFile(String filename, List<String> lines)
			throws IOException {
		...
		Preconditions
				.checkArgument(
						...
						filename, vocabSize, lines.size() - 1);
		...
		for (int n = 1; n < lines.size(); n++) {
			...
			Preconditions
					.checkArgument(
							...
							filename, n, layerSize, values.length - 1);
		...
		Word2VecModelThrift thrift = new Word2VecModelThrift()
				.setLayerSize(layerSize).setVocab(vocab).setVectors(vectors);
